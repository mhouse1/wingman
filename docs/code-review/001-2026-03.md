...
First review: 2026-03-20. All items resolved in v1.5.3 (2026-03-24).
Items are grouped by severity and area. Each item references the exact file and line(s) to change.

**Next review:** add new items below following the same format. Check this file at the start of each session — the CLAUDE.md rule points here.
...
| 1.2 | Click-to thread never joined on shutdown | P1 | Resolved |
| 1.3 | `ThreadPoolExecutor` lifecycle not guaranteed | P1 | Resolved |
| 1.4 | Mission lock can be permanently stuck held | P1 | Resolved |
| 1.5 | `_background_ocr_lock` acquire blocks main loop indefinitely | P1 | Resolved |
| 2.1 | Hotkey handler timeout guard missing | P2 | Resolved |
| 2.2 | `mss` grab not guarded against monitor disconnect | P2 | Resolved |
| 2.3 | `click_grid_region` has no platform guard | P2 | Resolved |
| 2.4 | `get_frame()` shared `mss` instance thread contract undocumented | P2 | Resolved |
| 3.1 | Tight poll loop during sleep interval | P3 | Resolved |
| 3.2 | `import time` inside method body | P3 | Resolved |
| 4.1 | Duplicate import in `capture.py` | P4 | Resolved |
| 4.2 | ANSI codes without Windows terminal guard | P4 | Resolved |
| 4.3 | Emoji in log strings — encoding risk | P4 | Resolved |
| 4.4 | Typos in log/comment strings | P4 | Resolved |
| 4.5 | `restart_last_mission()` tri-state return undocumented | P4 | Resolved |
| 5.1 | No test for `cleanup()` path | P5 | Resolved |
| 5.2 | No test for hotkey registration failure | P5 | Resolved |
| 5.3 | No test for mission cancellation race | P5 | Resolved |
| 5.4 | No test for `get_frame()` failure (post fix 2.2) | P5 | Resolved |
| 6.1 | Monitor index defaults to 2 | P6 | Resolved |
| 6.2 | Region defaults assume 1920×1200, undocumented | P6 | Resolved |
