...
behavior and comes from `README.md` section 3 unchanged. `u` launches J20
when `mission.default_mission` is `j20` (`u` launches whichever mission that key
names, ADR 145) and, like every `u` launch (section 2.2, default 2), it skips
instead of preempting when a mission is already running.
...
```mermaid
flowchart TD
    A[Battle entry or the u hotkey with j20 configured] --> M[mission_j20 takes the mission lock]
